...
void print_grid(int *plate, int n){
    if (n < 60) {
        for(int i = 1; i <= n; i++){
            for(int j = 1; j <= n; j++){
...
int main(int argc, char* argv[])
{
    ...
    int *grid, *new_grid, *tmp_grid;

    int dim = 16; //Linear dimension of our grid - not counting ghost cells
    int maxIter = 1; //Number of game steps 
    ...
    cudaMallocManaged(&grid, bytes);
    cudaMallocManaged(&new_grid, bytes);
    cudaMallocManaged(&tmp_grid, bytes);
    ...
    for (iter = 0; iter<maxIter; iter++) {
        printf("Iterations: %d\n", iter); 
        ghostRows<<<cpyGridRowsGridSize, cpyBlockSize>>>(dim, grid);
        ghostCols<<<cpyGridColsGridSize, cpyBlockSize>>>(dim, grid);
